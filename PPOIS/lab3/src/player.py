...
class Player(pygame.sprite.Sprite):
    def __init__(self, screen, x, y):
        ...
        self.gun = []
        for i in range(20):
            self.gun.append(pygame.image.load(utilities.get_image(f'survivor-move_handgun_{i}.png')))

        self.shotgun = []
        for i in range(20):
            self.shotgun.append(pygame.image.load(utilities.get_image(f'survivor-move_shotgun_{i}.png')))

        self.machine = []
        for i in range(20):
            self.machine.append(pygame.image.load(utilities.get_image(f'survivor-move_rifle_{i}.png')))
        ...
        self.image = pygame.image.load(utilities.get_image(self.player_weapon.image)).convert_alpha()
        self.image = pygame.transform.scale(self.image, (40, 40))
        self.rect = self.image.get_rect()
        self.rect.center = (self.x, self.y)
    ...
    def update(self, enemies, powers):
        ...
        self.angle = utilities.angle_between_points(self.x, self.y, mouse_x, mouse_y)
        ...
        image_to_draw, image_rect = utilities.get_rotated_image(self.image, self.rect, self.angle)
    ...
    def change_weapon(self, key):
        self.image = pygame.image.load(utilities.get_image(self.player_weapon.image))
